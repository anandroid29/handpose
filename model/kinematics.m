...
active_joints_indices = 1:12; 
active_cp_indices = [1, 2, 3]; 

WRIST_PIVOT = wrist_center + [-20; -5; 0]; %variable
...
function err = constrained_solver_step(q_active, active_idx, q_template, hand_model, target_pts, active_cp_idx)
    ...
    if isfield(hand_model, 'qmin') && isfield(hand_model, 'qmax')
        ...
        penalty_limits = 1e6 * sum(violation_min.^2 + violation_max.^2); %variable
    end
    ...
    tracking_error = 1e4 * sum(diff_vector.^2); %variable
